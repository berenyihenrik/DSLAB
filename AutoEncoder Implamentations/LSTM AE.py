...
train_losses, val_losses = train_model(model, train_loader, val_loader, optimizer, loss_function, scheduler, num_epochs=1)
...
def evaluate_lstm(model, test_loader, device, loss_fn, percentile_threshold=90):
    model.eval()
    anomaly_scores = []
...
anomalies = evaluate_lstm(model, test_loader, device, loss_function, 90)
